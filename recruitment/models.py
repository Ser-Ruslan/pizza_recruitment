...
class Interview(models.Model):
    ...
    scheduled_by = models.ForeignKey(User,
                                     on_delete=models.SET_NULL,
                                     null=True,
...
class ApplicationComment(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.pk:  # If creating new comment
            ...
            if author_profile.role in [
                    UserRole.HR_MANAGER, UserRole.RESTAURANT_MANAGER
            ]:
...
class QuickApplication(models.Model):
    vacancy = models.ForeignKey(Vacancy,
                                on_delete=models.CASCADE,
                                related_name='quick_applications')
    ...
    status = models.CharField(max_length=20,
                              choices=ApplicationStatus.choices,
                              default=ApplicationStatus.NEW)
    test_token = models.CharField(max_length=50,
                                  blank=True,
                                  null=True,
                                  unique=True)
    user_created = models.ForeignKey(User,
                                     ...
                                     null=True,
                                     blank=True,
                                     related_name='created_from_quick_apps')
...
class Test(models.Model):
    position_type = models.OneToOneField(PositionType,
                                         on_delete=models.CASCADE,
                                         related_name='test')
...
class UserAnswer(models.Model):
    attempt = models.ForeignKey(TestAttempt,
                                on_delete=models.CASCADE,
